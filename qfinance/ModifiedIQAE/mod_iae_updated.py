...
from qfinance.ModifiedIQAE.amplitude_estimator import AmplitudeEstimator, AmplitudeEstimatorResult
from qfinance.ModifiedIQAE.estimation_problem import EstimationProblem
...
class ModifiedIterativeAmplitudeEstimation(AmplitudeEstimator):
    ...
    def estimate(
        ...
        # the simulator is reinitialised here GPU support. Passing down the sampler from 
        # external files seem to cause a memory leak if GPU is used.
        ...
        while theta_intervals[-1][1] - theta_intervals[-1][0] > 4 * self._epsilon / np.pi:
            ...
            while powers[num_iterations - 1] == k:
                ...
                if self._confint_method == "beta":
                    ...
                    if self._check_early(shots, alpha_i, nu, theta_intervals, self._epsilon):
                        ...
                        if self._sampler:
                            ...
                            self._sampler.set_options(shots=self._sampler_shots)
                ...
                if self._sampler:
                    ...
                    job = simulator.run(gpu_circuit, shots=self._sampler_shots)
                    result = job.result()
                    # result = self._sampler.run(circuit, shots=self._sampler_shots).result()
                    
                    # quasi_dist = result.quasi_dists[0]
                    # counts = {}
                    # for key, value in quasi_dist.binary_probabilities().items():
                    #     counts[key] = int(value*self._sampler_shots)
