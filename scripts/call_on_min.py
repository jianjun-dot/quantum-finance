...
strike_prices = [round(step/n_steps * (0.9*high-1.1*low) + 1.1*low, 2) for step in range(n_steps)]
...
for (index, strike_price) in tqdm(enumerate(strike_prices), leave=False):
    ...
    for i in range(len(uncertainty_model.probabilities)):
        exact_value += uncertainty_model.probabilities[i]*max(0, min(uncertainty_model.values[i][0], uncertainty_model.values[i][1])-strike_price)
